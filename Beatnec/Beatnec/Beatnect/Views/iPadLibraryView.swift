...
struct iPadLibraryView: View {
    ...
    private var albums: [Album] {
        ...
        for track in playerService.libraryTracks {
            let key = "\(track.displayAlbum)|\(track.displayArtist)"
            if var a = dict[key] { a.tracks.append(track); dict[key] = a }
            else { dict[key] = Album(name: track.displayAlbum, artist: track.displayArtist, artworkUrl: track.fullArtworkUrl, tracks: [track]) }
        }
